        ...
        if (chapters.isNotEmpty() && chapters.first().date_upload == 0L) {
            val date =
                document
                    ...
                    .attr("datetime")
            if (date.isNotEmpty()) chapters.first().date_upload = parseDate(date)
        }

        return if (chapters.isNotEmpty()) chapters.reversed() else chapters
    }

    private fun parseDate(date: String): Long =
        try {
            // Try the format used in the parent class
